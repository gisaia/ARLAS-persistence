...
if [ -z "${CP}" ]; then
  CP="./arlas-persistence-server.jar"
fi

java -Xmx${ARLAS_XMX} -XX:+ExitOnOutOfMemoryError ${JVM_OPTION} -cp "${CP}" io.arlas.persistence.server.app.ArlasPersistenceServer server /opt/app/configuration.yaml
